...
class BuildSearchResults {
    ...
    public DefaultUrl: any = this.RootURL + this.UserURL;

    private ResultsGridBody: string = "";
    ...
    public async GetUserRepositories (reposURL:string):Promise<any>
    {
        ...
        let returnVal = await fetch(  // Using the fetch command to asynchronously call the web api.
            reposURL,
         { method: 'GET' }
        )
    ...
    public async GetUserHeader(userName:string) // Using a promise to call the web api. Then get the response before pushing the output to be displayed
    {
